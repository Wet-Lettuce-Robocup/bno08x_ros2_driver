...
BNO08xROS::BNO08xROS() : Node("bno08x_ros") {
  ...
  if (publish_magnetic_field_) {
    ...
    RCLCPP_INFO(this->get_logger(), "Magnetic Field Rate: %d",
                magnetic_field_rate_);
  }
  ...
  if (this->imu_rate_ < this->magnetic_field_rate_) {
    this->poll_timer_ = this->create_wall_timer(
        std::chrono::milliseconds(1000 /
                                  this->magnetic_field_rate_), // Hz to ms
  ...
  watchdog_->set_callback([this]() {
    RCLCPP_ERROR(
        this->get_logger(),
        "Watchdog timeout! No data received from sensor. Resetting...");
    this->reset();
  });
  watchdog_->start();
...
void BNO08xROS::init_comms() {
  ...
  if (i2c_enabled) {
    ...
    try {
      comm_interface_ =
          new I2CInterface(device, std::stoi(address, nullptr, 16));
    } catch (const std::exception &e) {
      RCLCPP_ERROR(this->get_logger(), "Failed to create I2CInterface: %s",
                   e.what());
    ...
    try {
      ...
      RCLCPP_ERROR(this->get_logger(), "UART Interface not implemented: %s",
                   e.what());
    ...
    try {
      ...
      RCLCPP_ERROR(this->get_logger(), "SPI Interface not implemented: %s",
                   e.what());
...
void BNO08xROS::init_parameters() {
  ...
  this->get_parameter("publish.magnetic_field.enabled",
...
void BNO08xROS::init_sensor() {
  ...
  if (publish_magnetic_field_) {
    if (!this->bno08x_->enable_report(
      ...
      RCLCPP_ERROR(this->get_logger(),
                   "Failed to enable magnetic field sensor");
    }
  }
  if (publish_imu_) {
    if (!this->bno08x_->enable_report(SH2_ROTATION_VECTOR,
      ...
      RCLCPP_ERROR(this->get_logger(),
                   "Failed to enable rotation vector sensor");
    }
...
void BNO08xROS::sensor_callback(void *cookie, sh2_SensorValue_t *sensor_value) {
  ...
  case SH2_MAGNETIC_FIELD_CALIBRATED:
    ...
    this->mag_msg_.header.stamp.nanosec =
        this->get_clock()->now().nanoseconds();
    // IMU will still return infrequent magnetic field reports even if the
    // report was not enabled, so check it was enabled before publishing.
    if (publish_magnetic_field_) {
  ...
  if (imu_received_flag_ == (ROTATION_VECTOR_RECEIVED | ACCELEROMETER_RECEIVED |
    ...
    this->imu_msg_.header.stamp.nanosec =
        this->get_clock()->now().nanoseconds();
